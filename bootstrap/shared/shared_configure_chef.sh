...
bootstrap_node="${MACHINE_NAME_PREFIX}bootstrap"
...
unset debpath
do_on_node $bootstrap_node "$CHEF_SERVER_INSTALL_CMD \
...
do_on_node $bootstrap_node "sudo /opt/opscode/embedded/bin/gem install $FILECACHE_MOUNT_POINT/knife-acl-0.0.12.gem \
...
do_on_node $bootstrap_node "sudo apt-get update \
  && cd \$HOME/chef-bcpc \
...
do_on_node $bootstrap_node "$KNIFE cookbook upload -a \
...
for vm in $vms $mon_vms; do
  ...
  do_on_node $bootstrap_node "$KNIFE bootstrap -x vagrant -P vagrant --sudo 10.0.100.1${i}"
  i=`expr $i + 1`
done

# augment the previously configured nodes with our newly uploaded environments and roles
ENVIRONMENT_SET=""
for vm in $bootstrap_node $vms $mon_vms; do
  ENVIRONMENT_SET="$ENVIRONMENT_SET $KNIFE node environment set bcpc-${vm#$MACHINE_NAME_PREFIX*}.$BCPC_HYPERVISOR_DOMAIN $BOOTSTRAP_CHEF_ENV && "
done
ENVIRONMENT_SET="$ENVIRONMENT_SET :"
do_on_node $bootstrap_node $ENVIRONMENT_SET

do_on_node $bootstrap_node "$KNIFE node run_list set bcpc-bootstrap.$BCPC_HYPERVISOR_DOMAIN 'role[BCPC-Hardware-Virtual],role[BCPC-Bootstrap]' \
...
for vm in $bootstrap_node vm1 $mon_vms; do
  ADMIN_SET="$ADMIN_SET $KNIFE group add actor admins bcpc-${vm#$MACHINE_NAME_PREFIX*}.$BCPC_HYPERVISOR_DOMAIN && "
done
ADMIN_SET="$ADMIN_SET :"
do_on_node $bootstrap_node $ADMIN_SET
...
if [[ $BOOTSTRAP_CHEF_DO_CONVERGE -eq 0 ]]; then
  for vm in $mon_vms; do
    do_on_node $bootstrap_node "$KNIFE node run_list set bcpc-${vm#$MACHINE_NAME_PREFIX*}.$BCPC_HYPERVISOR_DOMAIN 'role[BCPC-Monitoring]'"
  done
  echo "BOOTSTRAP_CHEF_DO_CONVERGE is set to 0, skipping automatic convergence."
  exit 0
else
  # run Chef on each node
  do_on_node $bootstrap_node "sudo chef-client"
  for vm in $vms; do
    do_on_node $vm "sudo chef-client"
  done
  # run on head node one last time to update HAProxy with work node IPs
  do_on_node ${MACHINE_NAME_PREFIX}vm1 "sudo chef-client"
